...
describe("XML attacks through the tool boundary", () => {
  it(
    "XXE and billion-laughs are rejected as KONTOR-XML-DTD; nothing is resolved or expanded",
    async () => {
  ...
  it(
    "parameter-entity XXE via content_base64 is rejected the same way",
    async () => {
  ...
  it(
    "deeply nested XML is rejected by depth, not by stack overflow",
    async () => {
      const depth = 5000;
  ...
  it(
    "oversized content_base64 is refused before parsing (KONTOR_MAX_FILE_MB)",
    async () => {
...
describe("path hygiene (file_path)", () => {
  it(
    "rejects traversal, relative, directory, wrong extension and special files with a clean message",
    async () => {
  ...
  it(
    "a .xml symlink to a non-XML secret is read but never echoed back",
    async () => {
...
describe("output_path hygiene (generate_invoice / convert_invoice)", () => {
  ...
  it(
    "never overwrites, never writes to a directory, a wrong extension or a relative path (parent dirs are created by design)",
    async () => {
...
describe("PDF attacks", () => {
  it(
    "encrypted PDF, fake PDF and garbage are rejected cleanly",
    async () => {
