...
describe("sovereignty: the whole MCP surface runs with the network blocked", () => {
  it("every tool is called at least once (matrix covers all documented targets)", async () => {
    const calls: Array<[string, Record<string, unknown>]> = [
      ...
      ["convert_invoice", { file_path: sample("valid-xrechnung-ubl.xml"), target: "html-preview" }],
  ...
  it("every resource, resource template instance and prompt", async () => {
